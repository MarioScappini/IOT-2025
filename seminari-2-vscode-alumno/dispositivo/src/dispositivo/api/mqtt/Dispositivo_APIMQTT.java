...
public class Dispositivo_APIMQTT implements MqttCallback {
	...
	public void messageArrived(String topic, MqttMessage message) throws Exception {
		...
		if (topicNiveles.length == 3 && topicNiveles[2].equalsIgnoreCase("comandos")) {
			if (payload.toLowerCase().contains("habilitar")) {
				dispositivo.habilitar();
			} else if (payload.toLowerCase().contains("deshabilitar")) {
				dispositivo.deshabilitar();
			} else {
				MySimpleLogger.warn(this.loggerId, "Acción '" + payload + "' no reconocida para el dispositivo.");
			}
			return;
		}
		...
		try {
			// Parsear JSON con campo "accion"
			org.json.JSONObject payloadJson = new org.json.JSONObject(payload);
			String action = payloadJson.getString("accion");
			...
			if (action.equalsIgnoreCase("encender"))
				...
				MySimpleLogger.warn(this.loggerId, "Acción '" + action + "' no reconocida.");
		} catch (org.json.JSONException e) {
			MySimpleLogger.warn(this.loggerId, "Payload no es un JSON válido: " + payload);
		}
	...
	public void iniciar() {
		...
		// Suscribirse a los comandos generales del dispositivo
		this.subscribe("dispositivo/" + this.dispositivo.getId() + "/comandos");

		// Asociar el publicador a cada función
		for (IFuncion f : this.dispositivo.getFunciones()) {
			if (f instanceof FuncionPi4Jv2) {
				((FuncionPi4Jv2) f).setPublicador(new FuncionPublisher_APIMQTT(myClient, f, dispositivo.getId()));
			}
